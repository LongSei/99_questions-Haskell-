...
dist :: (Eq a) => Graph a -> Int
dist (Graph _ xs) = foldl (\acc (a, b, c) -> acc + c) 0 xs

finding :: (Eq a) => [Graph a] -> Graph  a -> Graph a
finding (x:xs) nw
    | null xs = if dist x < dist nw then x else nw
    | dist x < dist nw = finding xs x
    | otherwise = finding xs nw

prim :: (Eq a) => Graph a -> Graph a
prim xs = finding p (head p)
    where p = spanningtree xs
